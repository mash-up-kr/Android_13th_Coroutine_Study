...
class GitHubRepositoryImpl @Inject constructor(
    ...
    override fun getUserInfoList(query: String, page: Int): Flow<UserListResponse> {
        return flow {
            searchDataSource.getUserInfoList(query = query, page = page).collect { userListResponse ->
                if (userListResponse != null) {
                    val flow = userListResponse.items.asFlow().flatMapMerge {
                        flow {
                            ...
                            val followerFlow = followerDataSource.getFollowers(userName = it.login)
                    ...
                    emit(UserListResponse(items = flow, total_count = userListResponse.total_count))
                } else {
